...
Starts the HTTP JSON-RPC server (default `http://127.0.0.1:8555`) and the MCP-over-stdio server against one shared `SessionManager`, plus the background chain-tip follower. This is what an agent harness should actually launch (over stdio) — see the `bin` entry under Architecture for config env vars and the ingest crate's documented v1 limitation.
...
- `ingest` — `ChainTipFollower` polls the upstream RPC on a fixed interval (`FORKYARD_INGEST_POLL_SECS`, default 12s) and pushes the result into `SessionManager::set_block_env`, so new forks keep picking up a real, moving block instead of whatever was current when the process started. **v1 scope limitation, documented in the crate and must be fixed before production:** this only refreshes the block context (number/timestamp/base fee) — it does not invalidate or refresh anything `forkyard-fetch`'s `SharedBackend` has already cached (account balances, nonces, storage, code), which stay pinned to their first-fetched values for the life of the process. Fine for a local/dev instance; a real correctness gap for any long-lived shared deployment.
...
1. **Chain-tip ingestion** — `forkyard-ingest` polls the upstream RPC for its latest block on a fixed interval, refreshing the shared block context new sessions fork against (see the `ingest` bullet above for its documented v1 limitation — block context only, not cache invalidation).
