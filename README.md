...
- `api-http` — a minimal Ethereum JSON-RPC server (`axum`) fronting a `SessionManager`: `POST /session` opens a session, `POST /session/{id}` carries `eth_chainId`, `eth_getBalance`, `eth_getTransactionCount`, `eth_sendRawTransaction` (legacy txs), plus a test-only `forkyard_setBalance` cheatcode. Lets external clients (`cast`, `alloy`, a wallet) share one warm cache over the wire — the production MCP/SDK path (`api-mcp`) stays in-process instead, to avoid paying this crate's JSON/HTTP cost on the hot path.

  **One server, many URLs, not one URL.** The base address (`http://host:port`) is the shared fork/cache; each session's actual endpoint is `http://host:port/session/{id}`, and those are genuinely isolated — verified live: funding the same address with different balances in two sessions and reading it back via each session's own URL returns the two different values. Pointing a real wallet (MetaMask) at a session URL: balance reads work, but the send-transaction confirmation flow calls `eth_estimateGas`/gas-fee methods first, which aren't implemented — sending via a wallet's own UI stalls there even though `eth_sendRawTransaction` itself works if you construct the signed tx yourself. `crates/api-http/examples/serve_demo.rs` runs a session server indefinitely for exactly this kind of manual `curl`/wallet poking.
