...
**RunPod & Modal — checked, not chosen.** Both are GPU-first platforms; this workload has no GPU need, so their core differentiator doesn't apply, and CPU is a secondary offering on either. Modal's own published rates make the mismatch concrete: standard Functions run $0.0000131/core-second + $0.00000222/GiB-second — an always-on 2 vCPU / 4 GiB container, the same size as the AWS comparison above, comes to **~$92/mo**, worse than Fargate's equivalent (~$72/mo) and far worse than raw EC2 (~$25/mo) or Hetzner (~$5/mo) for the same spec. `min_containers` can keep it always-warm, but that's paying full serverless per-second pricing for a workload that never scales to zero — none of that model's benefit, all of its premium. RunPod's CPU pods exist but aren't what the platform is priced to win on (GPU rental is); exact rates sit behind their console rather than published, so no confident same-size number here, but nothing found suggests they'd beat a dedicated host for plain CPU/RAM either.
